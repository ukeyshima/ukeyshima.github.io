...
const render = (gl, color, uniLocation) => {    
    length(color)<0.01?gl.clearColor(1.0,1.0,1.0,0.0):gl.clearColor(0.0,0.0,0.0,0.0);
...
const webGLStart = (canvas, gl, vs, fs, color) => {
    ...
    const drawColorProgram = create_program(create_shader(vs, gl.VERTEX_SHADER), create_shader(fs, gl.FRAGMENT_SHADER));
    const position = [
        -1.0, 1.0, 0.0,
        1.0, 1.0, 0.0,
        -1.0, -1.0, 0.0,
...
class OpacityPropertyArea extends React.Component {
    ...
    handleMouseDown(e) {
        ...
        gl.readPixels(event.offsetX, this.props.style.height - event.offsetY, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, u8);        
        this.props.handlechange(this.state.color.map(e=>e*255).concat(u8[3]/255));
        this.setState({
