class UndoManager {
    constructor(initialImg, initialState) {
        this.branchId = 0;
        this.undoStack = [this.undoStackObj(null, initialImg, initialState, this.branchId, null, [], 1)];
        this.redoStack = [];
    ...
    execute(delta, img, state) {
        ...
        this.undoStack.push(this.undoStackObj(delta, img, state, this.branchId, lastUndoStack, [], 1));
        lastUndoStack.nextStack.push(this.lastUndoStack());
        if (lastUndoStack.nextStack.length > 1) this.updateStackWidth(lastUndoStack);
        this.id++;
    ...
    unbra() {
        ...
        const branchPointStack = undoStack.concat().filter(e => e.nextStack.length > 1);
        const currentId = branchPointStack.map(e => e.branchId);
        currentId.push(lastUndoStack.branchId);
        const currentBranchId = currentId.concat().reverse().find(e => e > 0);
    ...
    rebra() {
        ...
        currentId.push(lastUndoStack.branchId);
        let nextBranchId = 0;
        const branchPoint = branchPointStack.concat().reverse().find((e, i) => {
