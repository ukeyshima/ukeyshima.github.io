...
float perlinNoise(vec3 p){
    ...
    vec3 j=fract(p);
    j=j*j*(3.0-2.0*j);
	return mix(mix(mix(dot(f1,k1),dot(f2,k2),j.x),mix(dot(f3,k3),dot(f4,k4),j.x),j.y),mix(mix(dot(f5,k5),dot(f6,k6),j.x),mix(dot(f7,k7),dot(f8,k8),j.x),j.y),j.z)*0.88+0.12;
}
...
float dist(vec2 p){
    float a=2.0;
    return sqrt(pow(p.x,a)+pow(p.y,a));
}
...
void main(void){    
    ...
    vec3 color=vec3(0.0);
    vec3 c=vec3(0.0,0.0,0.0+time);
    for(float i=0.0;i<5.0;i++){          
        color+=octavePerlinNoise(c)*vec3(0.8,0.2,0.001)*(50.0/pow(length(p),5.0));
        c+=ray;
    }    
