...
vec4 glitch(vec2 p){
   ...
   float t=time-mod(time,0.3);
    ...
    c.rgb-=random(vec2(mod(time,1.5)))<0.3 && random(q)<0.01 ? vec3(0.0):texture(tex,p+vec2(random(q),0.0)).rgb;
    c.xyz-=random(vec2(mod(time,1.5)))<0.3?vec3(0.0):vec3(0.3*random(vec2(0.0,p.y+time/10.0)));
    c.xyz-=random(vec2(0.0,p.y-time/5.0-mod(p.y-time/5.0,0.02)))>0.9?texture(tex,p+vec2(0.0,random(q))).rgb:vec3(0.0);
    return c;
}
void main(void){
