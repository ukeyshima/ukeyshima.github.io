...
vec4 glitch(vec2 p){
   ...
   float t=time-mod(time,0.3);
    ...
    c.rgb+=random(vec2(mod(time,1.5)))<0.3?vec3(0.0):vec3(random(q)*2.0,0.2,random(vec2(q.y,q.x))*2.0-1.0);
    c.rgb-=random(vec2(mod(time,1.5)))<0.3 && random(q)<0.01 ? vec3(0.0):texture2D(tex,p+vec2(random(q)*2.0-1.0,0.0)).rgb;
    ...
    c.rgb*=texture2D(tex,p-vec2(sin(p.y*50.0)/5.0*sin(time*3.0*p.y),-time/10.0)).rgb;
    return c;
}
void main(void){
